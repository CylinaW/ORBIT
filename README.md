...
Three quantities derived from the attention tensor are used throughout the paper, and the distinction matters when reproducing or extending results. The **per-nucleus, per-head map** $A^{(i,h)} \in \mathbb{R}^{P \times P}$ is the raw post-softmax attention from the final transformer block; the **head-averaged map** $\tilde{A}^{(i)} = \frac{1}{H} \sum_h A^{(i,h)}$ is used inside the intervention-consistent loss with gradient flow to the final block's query and key parameters; and the **reported attention matrix** $\bar{A}_{p,q} = \frac{1}{NH} \sum_{i, h} A^{(i,h)}_{p,q}$ is the dataset-level mean, computed post-softmax, **not symmetrized**, taken from the final transformer block. *Ā* is the matrix shown in every heatmap and used in every downstream analysis.
