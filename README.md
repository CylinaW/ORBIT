...
- **Determinism caveat.** PyTorch's CUDA kernels are not bitwise-deterministic across GPU architectures even with `torch.use_deterministic_algorithms(True)`; results within ± 0.001 macro-F1 are expected across A100 / H100 / RTX 6000 hardware. The CPU synthetic benchmark is fully deterministic.
